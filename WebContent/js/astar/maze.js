(function(g, undefined){
	...
	Grid.prototype.load = function(url){
		var self = this;
	...
	Grid.prototype.generate = function(width, height, dw, dh){
		this.width = width;
		this.height = height;
		this.dw = dw;
		this.dh = dh;
	...
	Grid.prototype.draw = function(stack){
		...
		while(unvisited > 0){
			...
			if(neighbors.length > 0){
				stack.push(current);
				var r = g.Utils.getRandomInt(0, neighbors.length - 1);
				var c = neighbors[r];
	...
	Grid.prototype.removeWall = function(a, b){
	...
	Grid.prototype.findPath = function(){
		var aStar = new g.AStar(this);
		var p = aStar.findPath();
		var path = new g.Path(p);
		this.path = path;
		//console.log(p);
	...
	Grid.prototype.getNeighbors4 = function(cell){
		var neighbors = [];
